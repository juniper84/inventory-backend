...
async function main() {
  ...
  if (process.env.CONFIRM !== 'YES') {
    console.log('Set CONFIRM=YES to remove Phase F test data.');
    return;
  }

  const prisma = new PrismaService();
  const businesses = await prisma.business.findMany({
    where: { name: { startsWith: 'Phase F Offline Test' } },
    select: { id: true },
  });

  if (!businesses.length) {
    console.log('No Phase F test businesses found.');
    return;
  }
  ...
  console.log(
    'Phase F test data removed (audit logs retained; businesses marked DELETED).',
  );
}
